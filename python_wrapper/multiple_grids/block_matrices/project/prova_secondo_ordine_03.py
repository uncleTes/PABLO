...
class Laplacian2D(BaseClass2D.BaseClass2D):   
    ...
    def modify_rhs(self):
        ...
        for i in xrange(0, n_oct):
            if i in ids_octree_contained:
                index = i + o_ranges[0]
                rhs_index = self.mask_octant(index)
                if (rhs_index != -1):
                    rhs_value = self._rhs.getValue(rhs_index)
                    center = octree.get_center(index)[:2]
                    add_to_rhs = (-1*ExactSolution2D.ExactSolution2D.solution(center[0], center[1])) +\
                                  (ExactSolution2D.ExactSolution2D.s_s_der(center[0], center[1]))
                    final_value = rhs_value + add_to_rhs
                    self._rhs.setValue(rhs_index, final_value)
