...
class Laplacian2D(BaseClass2D.BaseClass2D):   
        ...
        if grid:
            for i, center in enumerate(c_neighs):
                ...
                if check:
                    ...
                    # matrix.
                    b_values[i] = 0.0
                    #self._rhs.setValue(b_indices[i], 0)
	    
        #b_values[:] = [b_value * (0.0 / h2) for b_value in b_values]
        ...
        for octant in xrange(0, n_oct):
            ...
            if is_background:
                is_penalized = utilities.check_oct_into_squares(center ,
                                                  	        ...
                                                  	        logger ,
                                                  	        log_file)
                local_idx = octree.get_point_owner_idx((0.2499,
                                                        0.2501))
                global_idx = local_idx + o_ranges[0]

                if (global_idx in ids_octree_contained) and (global_idx == octant):
                    self._g_own = self.mask_octant(global_idx)
                    indices.append(self._g_own)
                    values.append(1.0)
                    new_value = ExactSolution2D.ExactSolution2D.solution(0.2499, 0.2501)
                    print("sostituendo valore soluzione esatta termine noto : vecchio valore " + str(self._rhs.getValue(self._g_own)) +\
                          " nuovo valore : " + str(new_value))
                    self._rhs.setValue(self._g_own, new_value)
                    #self.assembly_petsc_struct("rhs")
                    found_row = True
                else:
    ...
    def solve(self):
        ...
        self._b_mat.mult(self._rhs, self._sol)
    ...
    def update_bg_grids(self    ,
        ...
        (x_center, y_center) = (0.2499, 0.2501)
	#h2 = key[3] * key[3]
        local_idx = octree.get_point_owner_idx((x_center,
                                                y_center))
        ...
        if global_idx in ids_octree_contained:
            ...
            n_b_c= [coeff * (1.0) for coeff in n_b_c]
            #self.apply_rest_prol_ops(key[1],
            #                         n_n_i ,
            #                         n_b_c ,
            #                         n_c)
            self.apply_rest_prol_ops(self._g_own,
                                     n_n_i ,
                                     n_b_c ,
                                     n_c)
