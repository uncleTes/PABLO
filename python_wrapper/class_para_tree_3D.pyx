...
# --------------------TYPEDEFS D2-------------------
ctypedef public vector[Class_Octant[D3]] OctantsType 
...
# Wrapper Python for class Class_Para_Tree<3>
cdef class  Py_Class_Para_Tree:
	# Pointer to the object Class_Para_Tree<3>
	cdef Class_Para_Tree[D3]* thisptr

	# ------------------------------Constructor-----------------------------
	# different number of arguments can be passed, so different 
	# Class_Para_Tree<3> constructors can be called
	def __cinit__(self, *args):
		number_of_parameters = len(args)
		
		if (number_of_parameters == 0):
			self.thisptr = new Class_Para_Tree[D3]("PABLO.log")
		elif (number_of_parameters == 1):
			self.thisptr = new Class_Para_Tree[D3](args[0])
			...
			self.thisptr = new Class_Para_Tree[D3](<double>args[0],
							       <double>args[1],
							       <double>args[2],
							       <double>args[3],
							       <ivector2D&>args[4],
	...
	def __dealloc__(self):
		# delete the pointer to the internal object Class_Para_Tree<3>
		del self.thisptr
	...
	#	if (from_octant):
	#		center = self.thisptr.getCenter(<Class_Octant[D3]*><void*>idx)
	#	else:
	...
	def set_balance(self, idx, bool balance):
		cdef Class_Octant[D3]* octant
		
		if (type(idx) is not int):
			oct = <Class_Octant[D3]*><void*>idx
			self.thisptr.setBalance(<Class_Octant[D3]*>octant, balance)
		else:
	...
	#	if (not from_index):
	#		self.thisptr.setMarker(<Class_Octant[D3]*><void*>octant, <int8_t>marker)
	...
	#	cdef Class_Octant[D3]* oct
	#	oct =  <Class_Octant[D3]*><void*>octant
	...
	#	if (type(idx) is not int):
	#		oct = <Class_Octant[D3]*><void*>idx
	#		return self.thisptr.getBalance(<Class_Octant[D3]*>oct)
	#	else:
	...
	#	return self.thisptr.getNodes(<Class_Octant[D3]*><void*>idx)
	...
	def for_test_bubbles(self, int iteration, int nrefperiter, int nocts, int nnodes, int nb, BB):
		...
		for iref in xrange(0, c_nrefperiter):
			for i in xrange(0, c_nocts):
				...
				while (not inside and ib < c_nb):
					(xc, yc, zc) = BB[ib].center
					(x_2, y_2, z_2) = (center[0] - xc, 
							   center[1] - yc,
							   center[2] - zc)
					radius = BB[ib].radius
					radius_sqr = radius*radius
					oct_data.push_back(99)
		...
		self.thisptr.writeTest("PabloBubble_iter" + str(iteration), oct_data)
