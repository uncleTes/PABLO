...
class build_ext(_build_ext):
	description = ("Custom build_ext \"pablo-include-path\"" +
			" \"mpi-include-path\"" + " \"opt-IO-include-path\"" +  
			" \"opt-base-include-path\"" + " \"extension-source\" command for Cython")

	# Getting "user_options" from build_ext
	user_options = _build_ext.user_options

	# Appending new "user_options": "pablo-lib-path" and "mpi-library-path"
        # and "opt-IO-include-path" and "extensions-source"
	user_options.append(("pablo-include-path=", "P", "PABLO include path"))
	user_options.append(("mpi-include-path=", "M", "mpi include path"))
        user_options.append(("opt-IO-include-path=", "O", "opt_IO include path"))
        user_options.append(("opt-base-include-path=", "B", "opt_base include path"))
	...
	# Find opt_IO include path
	def find_opt_IO_include_path(self):
		OPT_IO_INCLUDE_PATH = os.environ.get("OPT_IO_INCLUDE_PATH")

		if (OPT_IO_INCLUDE_PATH is None):
			print("Dude, no \"OPT_IO_INCLUDE_PATH\" environment" +
				" variable found. Please, check this out or" +
				" enter it via shell")

			sys.exit(1)

		return OPT_IO_INCLUDE_PATH
	
        # Find opt_base include path
	def find_opt_base_include_path(self):
		OPT_BASE_INCLUDE_PATH = os.environ.get("OPT_BASE_INCLUDE_PATH")

		if (OPT_BASE_INCLUDE_PATH is None):
			print("Dude, no \"OPT_BASE_INCLUDE_PATH\" environment" +
				" variable found. Please, check this out or" +
				" enter it via shell")

			sys.exit(1)

		return OPT_BASE_INCLUDE_PATH
	...
	def initialize_options(self):
		...
		self.mpi_include_path = None
                self.opt_IO_include_path = None
                self.opt_base_include_path = None
	...
	def finalize_options(self):
		...
		if (self.pablo_include_path is None):
			self.pablo_include_path = self.find_pablo_include_path()
                if (self.opt_IO_include_path is None):
                    self.opt_IO_include_path = self.find_opt_IO_include_path() 
                if (self.opt_base_include_path is None):
                    self.opt_base_include_path = self.find_opt_base_include_path() 
		...
		ext_modules = [Extension(os.path.splitext(self.extensions_source)[0],
					...
					extra_compile_args=["-std=c++11", 
							    ...
							    "-I" + self.pablo_include_path,
                                                            "-I" + self.opt_IO_include_path,
                                                            "-I" + self.opt_base_include_path],
					extra_link_args=["-fPIC"],
					cython_directives = {"boundscheck": False,
							     "wraparound": False},
					language="c++",
					extra_objects=["libPABLO.a", "libopt_IO.a", "libopt_base.a"],
					include_dirs=[".", 
						      self.pablo_include_path,
                                                      self.opt_IO_include_path,
                                                      self.opt_base_include_path,
